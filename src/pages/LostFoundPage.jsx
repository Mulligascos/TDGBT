...
// ─── REPORT FORM ─────────────────────────────────────────────────────────────
const ReportForm = ({ currentUser, courses, onSubmit, onClose }) => {
...
export const LostFoundPage = ({ currentUser, isAdmin, courses }) => {
  ...
  const [filter, setFilter] = useState('found'); // 'found' | 'claimed' | 'all'
  ...
  const unclaimed = discs.filter(d => d.status === 'found').length;
    ...
    <div style={pageStyle}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 3 }}>
            {unclaimed > 0 ? `${unclaimed} disc${unclaimed !== 1 ? 's' : ''} waiting to be claimed` : 'No unclaimed discs right now'}
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '20px 20px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 18 }}>
          {[
            { id: 'found',   label: `Unclaimed${unclaimed > 0 ? ` (${unclaimed})` : ''}` },
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            subtitle={filter === 'found' ? "Found a disc on the course? Tap '+ Found one' to report it" : 'Try a different filter'}
          />
        ) : (
          filtered.map(disc => (
            <DiscCard
