        ...
        {tab === 'members' ? (
          <>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {available.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No members available</p>
              ) : available.map(p => (
          ...
          <div style={{ flex: 1 }}>
            ...
            <button onClick={addGuest} disabled={!guestName.trim()} style={{
              ...
              background: guestName.trim() ? 'linear-gradient(135deg, rgba(74,222,128,0.2), rgba(74,222,128,0.1))' : 'var(--bg-input)',
              border: `1px solid ${guestName.trim() ? 'rgba(74,222,128,0.3)' : 'var(--border)'}`,
...
const PreRoundSetup = ({ round, course, currentUser, allPlayers, initialPlayers, onStart, onBack }) => {
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '24px 20px 120px' }}>
        ...
        {/* Round info */}
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-card)', borderRadius: 14, padding: '14px 16px', marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--text-secondary)' }}>
            <span>📍 {course.name}</span>
            <span>{round.total_holes} holes · Starting hole {round.starting_hole || 1}</span>
          </div>
        </div>
        ...
        {players.map(p => (
          <div key={p.id} style={{
            ...
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 14, fontWeight: 700, color: BRAND.light,
            }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 6 }}>
                {formatName(p.name)}
                {p.id === currentUser.id && <span style={{ fontSize: 10, color: BRAND.light, fontWeight: 700 }}>you</span>}
                {p.isGuest && <span style={{ fontSize: 9, background: 'rgba(148,163,184,0.2)', color: '#94a3b8', padding: '1px 5px', borderRadius: 4, fontWeight: 700 }}>GUEST</span>}
                {isJunior(p) && <span style={{ fontSize: 9, background: 'rgba(251,191,36,0.2)', color: '#fbbf24', padding: '1px 5px', borderRadius: 4, fontWeight: 700 }}>J</span>}
              </div>
              {p.bagTag && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>🏷️ #{p.bagTag}</div>}
            </div>
            {p.id !== currentUser.id && (
              <button onClick={() => removePlayer(p.id)} style={{
                padding: '6px 8px', borderRadius: 8, border: 'none', cursor: 'pointer',
                background: 'rgba(248,113,113,0.08)', color: '#f87171', flexShrink: 0,
              }}>
                <X size={14} />
              </button>
            )}
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          <button onClick={() => onStart(players)} style={{
            ...
            <Check size={18} /> Start Round with {players.length} {players.length === 1 ? 'Player' : 'Players'}
          </button>
        </div>
      </div>
...
const MiniScorecard = ({ players, scores, pars, currentHole, committedHoles }) => {
  const rows = players.map(p => {
    const playerScores = scores[p.id] || [];
    const adjusted = playerScores.map(s => s != null ? applyHandicap(s, p) : null);
    return { player: p, adjusted };
  });

  return (
    <div style={{
      background: 'var(--bg-card)', border: '1px solid var(--border)',
      borderRadius: 12, padding: '12px', marginBottom: 20, overflowX: 'auto',
    }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, fontFamily: "'DM Sans', sans-serif" }}>
        <thead>
          <tr>
            <td style={{ padding: '3px 6px', color: 'var(--text-secondary)', fontWeight: 700, whiteSpace: 'nowrap' }}>Hole</td>
            {pars.map((_, i) => (
              <td key={i} style={{
                padding: '3px 2px', textAlign: 'center', fontWeight: 700,
                color: i === currentHole ? BRAND.light : 'var(--text-secondary)',
                minWidth: 18,
              }}>{i + 1}</td>
            ))}
            <td style={{ padding: '3px 6px', textAlign: 'right', fontWeight: 700, color: 'var(--text-secondary)' }}>Tot</td>
          </tr>
          <tr>
            <td style={{ padding: '2px 6px', color: 'var(--text-muted)', fontSize: 10 }}>Par</td>
            {pars.map((p, i) => (
              <td key={i} style={{ padding: '2px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 10 }}>{p}</td>
            ))}
            <td style={{ padding: '2px 6px', textAlign: 'right', color: 'var(--text-muted)', fontSize: 10 }}>{totalPar(pars)}</td>
          </tr>
        </thead>
        <tbody>
          {rows.map(row => {
            // Only show scores for holes the player has committed (navigated away from)
            const committedScores = row.adjusted.map((s, i) =>
              committedHoles.has(i) ? s : null
            );
            const total = committedScores.filter(s => s != null).reduce((a, b) => a + b, 0);
            const holesCommitted = committedScores.filter(s => s != null).length;
            const vp = holesCommitted > 0 ? calcVsPar(committedScores, pars) : null;
            return (
              <tr key={row.player.id}>
                <td style={{ padding: '4px 6px', color: 'var(--text-secondary)', fontWeight: 600, whiteSpace: 'nowrap', fontSize: 11 }}>
                  {formatName(row.player.name).split(' ')[0]}
                </td>
                {row.adjusted.map((s, i) => {
                  const isCommitted = committedHoles.has(i);
                  const diff = isCommitted && s != null ? s - pars[i] : null;
                  return (
                    <td key={i} style={{
                ...
                <td style={{ padding: '4px 6px', textAlign: 'right', fontWeight: 700, color: vp != null ? vsParColor(vp) : 'var(--text-muted)', fontSize: 12 }}>
                  {holesCommitted > 0 ? vsParLabel(vp) : '—'}
                </td>
              </tr>
            );
          })}
    ...
    <div style={{ padding: '0 20px 20px' }}>
        ...
        {rows.map((row, i) => (
          <div key={row.player.id} style={{
            ...
            <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 800, color: vsParColor(row.vsPar), width: 36, textAlign: 'right', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ overflowX: 'auto', marginBottom: 20 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: "'DM Sans', sans-serif" }}>
          <thead>
            <tr>
              <td style={{ padding: '6px 8px', color: 'var(--text-secondary)', fontWeight: 700 }}>Hole</td>
              {pars.map((_, i) => (
                <td key={i} style={{ padding: '6px 4px', color: 'var(--text-secondary)', textAlign: 'center', fontWeight: 700 }}>{i + 1}</td>
              ))}
              <td style={{ padding: '6px 8px', color: 'var(--text-secondary)', textAlign: 'right', fontWeight: 700 }}>Tot</td>
            </tr>
            <tr>
              <td style={{ padding: '4px 8px', color: 'var(--text-secondary)', fontSize: 10 }}>Par</td>
              {pars.map((p, i) => (
                <td key={i} style={{ padding: '4px', color: 'var(--text-secondary)', textAlign: 'center', fontSize: 10 }}>{p}</td>
              ))}
              <td style={{ padding: '4px 8px', color: 'var(--text-secondary)', textAlign: 'right', fontSize: 10 }}>{totalPar(pars)}</td>
            </tr>
          </thead>
          <tbody>
            {rows.map(row => (
              <tr key={row.player.id}>
                ...
                <td style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 700, color: vsParColor(row.vsPar), fontFamily: 'Arial, sans-serif' }}>
                  {row.total}
                </td>
              </tr>
            ))}
...
const BagTagChallengeScreen = ({ result, course, currentUser, roundId, courseId, onComplete, updateUser }) => {
  ...
  const vpl = (v) => v === 0 ? 'E' : v > 0 ? `+${v}` : `${v}`;
  const vpc = (v) => v < 0 ? '#4ade80' : v === 0 ? '#fbbf24' : '#f87171';
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)', color: 'var(--text-primary)', paddingBottom: 60 }}>
      <div style={{ background: 'linear-gradient(135deg, #92400e, #d97706)', padding: '52px 20px 24px', borderBottom: '1px solid rgba(251,191,36,0.2)' }}>
        <div style={{ maxWidth: 520, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>Bag Tag Challenge!</div>
          <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 4 }}>{eligible.length} tagged players · {course?.name}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '24px 20px' }}>
        {hasTies && (
          <div style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 14, padding: '12px 16px', marginBottom: 20, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>Use the arrows to set the final finishing order for tied players. Tags are assigned top to bottom.</div>
            </div>
          </div>
        )}

        <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10 }}>Finishing Order</div>
        {order.map((p, i) => {
          ...
          return (
            <div key={p.id} style={{
              display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', marginBottom: 8, borderRadius: 14,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{formatName(p.name)}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>
                  {p.total_strokes} strokes · <span style={{ color: vpc(p.vs_par), fontWeight: 700 }}>{vpl(p.vs_par)}</span> · 🏷️ #{p.bagTag}
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                <div style={{ minWidth: 36, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: tagChanged ? 'rgba(248,113,113,0.15)' : 'var(--bg-input)', border: `1px solid ${tagChanged ? 'rgba(248,113,113,0.35)' : 'var(--border)'}` }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: tagChanged ? '#f87171' : 'var(--text-muted)', fontFamily: 'Arial, sans-serif' }}>#{p.bagTag}</span>
                </div>
                <div style={{ width: 22, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {tagChanged ? <span style={{ fontSize: 14, color: '#fbbf24' }}>➜</span> : <span style={{ display: 'block', width: 12, height: 2, borderRadius: 2, background: 'var(--text-muted)' }} />}
                </div>
                <div style={{ minWidth: 36, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: tagChanged ? 'rgba(74,222,128,0.15)' : 'var(--bg-input)', border: `1px solid ${tagChanged ? 'rgba(74,222,128,0.35)' : 'var(--border)'}` }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: tagChanged ? '#4ade80' : 'var(--text-muted)', fontFamily: 'Arial, sans-serif' }}>#{tagAfter}</span>
                </div>
              </div>
              {(canUp || canDown) && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                  <button onClick={() => moveUp(i)} disabled={!canUp} style={{ width: 28, height: 26, borderRadius: 6, border: 'none', cursor: canUp ? 'pointer' : 'default', background: canUp ? 'rgba(251,191,36,0.15)' : 'transparent', color: canUp ? '#fbbf24' : 'transparent', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>▲</button>
                  <button onClick={() => moveDown(i)} disabled={!canDown} style={{ width: 28, height: 26, borderRadius: 6, border: 'none', cursor: canDown ? 'pointer' : 'default', background: canDown ? 'rgba(251,191,36,0.15)' : 'transparent', color: canDown ? '#fbbf24' : 'transparent', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>▼</button>
                </div>
              )}
            </div>
          );
        ...
        <button onClick={() => onComplete?.()} style={{ width: '100%', padding: '12px', borderRadius: 12, background: 'transparent', border: '1px solid var(--border-card)', color: 'var(--text-muted)', fontFamily: "'DM Sans', sans-serif", fontSize: 13, cursor: 'pointer' }}>
...
export const StrokePlayScorer = ({ round, course, allPlayers, currentUser, onComplete, onBack, updateUser }) => {
  ...
  const [ctpSheet, setCtpSheet] = useState(false);
  const [ctpPos, setCtpPos] = useState(null);
  ...
  const submitCtpShot = async () => {
    ...
    if (!error) { setCtpSheet(false); setCtpPos(null); showCtpToast(`🎯 CTP recorded! ${fmtDist(dist)} from pin`); }
  };
  ...
  React.useEffect(() => {
    ...
    saveDraft(round.id, currentUser.id, {
      ...
      currentHole, cardPlayers, scores,
      visitedHoles: [...visitedHoles],
    });
  }, [scores, currentHole, cardPlayers, view]);  // eslint-disable-line
    ...
    <div style={pageStyle}>
        ...
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', fontFamily: 'Arial, sans-serif', lineHeight: 1 }}>{par}</div>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '16px 20px 0' }}>
        ...
        <MiniScorecard
          ...
          committedHoles={new Set([...visitedHoles].filter(h => h !== currentHole))}
        />
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          ...
          <div style={{ position: 'relative', background: 'var(--bg-nav)', borderRadius: '20px 20px 0 0',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)', fontFamily: "'Syne', sans-serif" }}>🎯 {ctpForHole.name}</div>
            ...
            <button onClick={captureCtpGps} disabled={ctpGpsLoading} style={{
              ...
              background: ctpPos ? 'rgba(74,222,128,0.1)' : 'var(--bg-input)',
              border: '2px solid ' + (ctpPos ? 'rgba(74,222,128,0.4)' : 'var(--border-card)'),
              color: 'var(--text-primary)', fontFamily: "'DM Sans', sans-serif",
              display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12,
            }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0,
                background: ctpPos ? 'rgba(74,222,128,0.2)' : 'var(--bg-input)',
                display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {ctpGpsLoading
