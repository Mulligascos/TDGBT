  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <button onClick={() => setView('create')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: `linear-gradient(135deg, ${BRAND.primary}, ${BRAND.accent})`, border: '1px solid rgba(74,222,128,0.3)', borderRadius: 10, color: '#ffffff', fontFamily: "'Syne', sans-serif", fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {seasons.length === 0 ? (
        ...
        <div key={s.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {s.status === 'active' && (
              <button onClick={() => closeSeason(s.id)} style={{ flex: 1, padding: '8px', borderRadius: 8, background: 'rgba(248,113,113,0.12)', border: '1px solid rgba(248,113,113,0.35)', color: 'var(--text-primary)', fontFamily: "'Syne', sans-serif", fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                🔒 Close Season
              </button>
            )}
          </div>
